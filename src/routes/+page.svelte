<script lang="ts">
	...
	let page = $state(data.gameDetails.scoreboardEnabled ? 'players' : 'tasks');
	let live = $state(true);
...
<svelte:head>
	<title>
		{data.gameDetails.name} - {data.gameDetails.subtitle}
	</title>
</svelte:head>

{#if data.gameDetails.active}
	<h1 class="absolute top-0 flex w-full flex-col items-center justify-center">
		...
		<HomeMenu
			...
			{tasksButtonVariant}
			{shopButtonVariant}
			shopEnabled={data.gameDetails.shopEnabled}
			tasksEnabled={data.gameDetails.tasksEnabled}
			scoreboardEnabled={data.gameDetails.scoreboardEnabled}
			bind:page
		/>
	</h1>
	...
	{:else if page === 'tasks'}
		<div class="mx-auto mt-32 w-full px-2 md:w-1/2 md:px-0">
			{#if !data.gameDetails.tasksEnabled}
				<EmptyCard message="Tasks are disabled." />
	...
	{:else if page === 'shop'}
		<div class="mx-auto mt-32 w-full px-2 md:w-1/2 md:px-0">
			{#if !data.gameDetails.shopEnabled}
				<EmptyCard message="The Shop is disabled." />
			{:else if data.cosmetics.length === 0}
	...
	{#if data.gameDetails.showInstagram}
...
{:else}
	<div class="mx-auto mt-32 w-full px-2 md:w-1/2 md:px-0">
		<EmptyCard message="This game is not yet active." />
	</div>
{/if}
